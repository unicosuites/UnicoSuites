...
(() => {
  ...
  // --------------------------------------------------------
  // Nav scroll-converge — logo & CTA glide inward + fade in,
  // centre links materialise, as the hero scrolls past.
  // Runs alongside (not instead of) the .is-scrolled background.
  // --------------------------------------------------------
  (function navConverge() {
    ...
    // Reduced motion: CSS never applies the pushed-out pre-state, so the nav
    // is already resting — do nothing. Heroless pages (privacy/cookies/404):
    // cancel the CSS pre-state and lock to final.
    if (reduceMotion) return;
    if (!hero || !brand) { setFinal(); return; }

    const COMPLETE_AT = 0.6;             // animation finishes at 60% of hero height
    const easeOut = t => t * (2 - t);    // quadratic ease-out
    const offsetFor = () => (window.innerWidth < 600 ? 22 : 40);

    let ticking = false;
    let locked  = false;                 // true once parked in final position
    ...
    const apply = () => {
      ...
      if (p >= 1) {
        if (locked) return;              // past hero — stop touching the DOM
        setFinal();
        locked = true;
        return;
      }
    ...
    apply();                             // set the scroll-0 state immediately
